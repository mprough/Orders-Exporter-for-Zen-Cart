...
Orders Exporter v3.0.0 is an encapsulated Zen Cart Plugin Manager package. Its executable and language files remain inside:

```text
files/zc_plugins/OrdersExporter/v3.0.0/
```
...
Version 3.0.0 uses two controls:
...
The export is available only through the authenticated Zen Cart admin page. Version 3.0.0 does not create server-side export files and does not require a writable catalog directory.
