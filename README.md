...
Version 3.0.0 is a complete modernization of the original Orders Exporter. It is encapsulated for Zen Cart Plugin Manager, makes no core-file edits, and processes large stores in controlled database batches instead of loading the complete export into PHP memory.
...
6. Find **Orders Exporter v3.0.0** and select **Install**.
...
Do not rename the `OrdersExporter` or `v3.0.0` directories.

## Upgrading from version 2.0 or an earlier release

Version 3.0.0 replaces the old loose admin files. It is not installed by copying files directly into the renamed admin directory.

1. Make a complete backup of the store files and database.
2. Check the old catalog-side `oexport` directory for saved exports that must be retained securely.
3. Install v3.0.0 through Plugin Manager using the installation instructions above.
...
The previous release did not create configuration database rows. Version 3.0.0 creates its own configuration group and admin menu registrations during installation.

The original v2.0 installation document is preserved at [`docs/archive/readme-v2.0.txt`](docs/archive/readme-v2.0.txt) for historical reference. Do not use those archived instructions to install v3.0.0.
...
Version 3.0.0 changes both parts of that process:
...
Version 3.0.0 improves export safety:
...
The former **Save Orders on server** feature was intentionally retired because it placed order data in a catalog-side directory. Version 3.0.0 only provides direct admin downloads.
...
- Confirm that v3.0.0 is installed under **Modules > Plugin Manager**.
