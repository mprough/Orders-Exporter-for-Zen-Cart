...
if ($mat_dlmethod == 'stream' or $mat_dlmethod == 'tempfile') {
    ...
    $result = $db->Execute($filelayout_sql);
    while (!$result->EOF) {
        // remove any bad things in the texts that could confuse the Export
        $therow = '';
        foreach( $filelayout as $key => $value ) {
            $thetext = $result->fields[$key];
            if (empty($thetext)) $thetext = ''; 
    ...
    if ($mat_dlmethod == 'stream') {     
        ...
        if ($fp) { 
            ...
            $messageStack->add(sprintf(ORDERSEXPORT_MSGSTACK_FILE_OPEN_FAILURE, $tempdir), 'error');
        }
    } 
